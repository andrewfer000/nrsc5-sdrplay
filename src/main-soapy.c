...
{
    fprintf(stderr, "Usage: %s [-v] [-q] [--am] [-l log-level] [-d Soapy-device-args] [-p ppm-error] [-g gain-name=gain-value...] [-w iq-output] [-o wav-output] [-A antenna] [--dump-hdc hdc-output] [--dump-aas-files directory] frequency program\n", progname);
}
...
static int parse_args(state_t *st, int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "w:o:d:p:g:ql:vA:", long_opts, NULL)) != -1)
    {
        switch (opt)
        {
        case 1:
        ...
        case 'p':
            ...
            break;
        case 'g':
            st->gain_settings = strdup(optarg);
            break;
        case 'q':
            log_set_quiet(1);
            break;
...
int main(int argc, char *argv[])
{
    ...
    if (st->ppm_error != 0 && nrsc5_set_freq_correction(radio, st->ppm_error) != 0)
    {
    ...
    if (st->gain_settings)
        nrsc5_set_gain(radio, st->gain_settings);
